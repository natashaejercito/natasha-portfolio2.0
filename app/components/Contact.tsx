    ...
    <section id="contact" style={{ background: 'var(--deep)', color: 'var(--invtext)' }}>
      <div style={{ maxWidth: 1180, margin: '0 auto', padding: 'clamp(72px,9vw,120px) clamp(20px,5vw,56px)' }}>
        ...
        <p style={{ fontSize: 'clamp(16px,1.6vw,19px)', color: 'rgba(250,247,240,0.72)', maxWidth: '50ch', margin: '22px 0 0', lineHeight: 1.55 }}>
          Check out my work and follow me on social media. I love connecting with new people and collaborating on exciting projects!
        </p>
          ...
          {LINKS.map((link, i) => (
            ...
            >
              ...
              <span style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 3 }}>
                <span style={{ fontSize: 'clamp(20px,2.6vw,32px)', fontWeight: 600 }}>{link.label}</span>
                <span style={{ fontFamily: "'Space Mono', monospace", fontSize: 12, color: 'rgba(250,247,240,0.6)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{link.sub}</span>
              </span>
